...
def match_site(sdk, idname, global_vars):
    ...
    print_array.append({ style: T1, theader: "Health Check for SITE: " + pUnderline(pBold(site_name)), data: [ 
        [ "Site ID" , str(site_dict['id']) ],
        [ "Status"  , str(site_dict["admin_state"]) ],
        [ "Description"  , str(site_dict["description"]) ],
        [ "Address"  , str(site_dict["address"]["street"]) + ", " + str(site_dict["address"]["city"]) +  " " + str(site_dict["address"]["state"]) +   ", " + str(site_dict["address"]["post_code"])  ],
        [ "Role"  , str(site_dict["element_cluster_role"]) ],
        ]   })
